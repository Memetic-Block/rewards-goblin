job "rewards-goblin-stage" {
  ...
  group "rewards-goblin-stage-group" {
    count = 1
    ...
    task "rewards-goblin-stage-task" {
      driver = "docker"
      ...
      env {
        ...
        AO_CHEESE_MINT_PROCESS_ID="mu9_WPdmWXvAGCP_aNzTVCYN0j62CY0VepzDPupyGk4"
      }
      ...
      vault { policies = [ "rewards-goblin-stage" ] }

      template {
        data = "{{ with secret `kv/wuzzy/rewards-goblin-stage` }}{{ base64Decode .Data.data.WALLET_JWK_BASE64 }}{{ end }}"
